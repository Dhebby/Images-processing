function Signal=exercise1(range, period, kparam, nsample)
%
%cosa fa la funzione
...
plot(timeInterval, func1), %title(['Function func1 with parameters:...'
	%range=['num2str(tmin),' 'num2str(tmax)'], period=',num2str(period),',
    %k=',num2str(kparam),', nsample='num2str(nsample)'])  %comando ke definisce l'array di plot
grid on;
%
subplot(1,3,2);
plot(timeInterval, func2), %title(['Function func2 with parameters:...'
	%range=['num2str(tmin),' 'num2str(tmax)'],period=',num2str(period),',
	%k=',num2str(kparam),', nsample='num2str(nsample)'])
grid on;
%
subplot(1,3,3);
plot(timeInterval, Signal), %title(['Signal with parameters:...'
	%range=['num2str(tmin),' 'num2str(tmax)'],period=',num2str(period),',
	%k='num2str(kparam)', nsample='num2str(nsample)'])
grid on;
%
